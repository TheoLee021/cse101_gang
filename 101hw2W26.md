...
$\{A, G, B, H\}, \{I\}, \{F\}, \{D\}, \{E\}, \{C, J, L, K\}$

(b) Which are source SCCs and which are sink SCCs?

Solution:

The source SCC is $\{C, J, L, K\}$

The sink SCC is $\{A, G, B, H\}$
...
---
## Question 2
A DAG $G$ is called semi-connected if for every pair of vertices $u$ and $v$, there is a path from $u$ to $v$ or a path from $v$ to $u$ (but not both.)

Consider the following algorithms that attempt to determine if a given DAG is semi-connected:
...
- Algorithm1 ($G$; a DAG $G$.)
	1. Run DFS algorithm on G.
	2. Let h be  the vertex with highest post number.
	3. Run explore on G starting from h
	4. if all vertices in G are visited:
	5.      return TRUE
	6. else
	7.      return FALSE
- Algorithm2 ($G$; a DAG $G$.)
	1. Run DFS algorithm on G.
	2. Organize the vertices in decreasing order by post numbers: $(v_1,\dots,v_n)$
	3. for $i=1,\dots,n-1$
	4.      if $(v_i,v_i+1)$ is not an edge:
	5.           return FALSE
	6.      else
	7.           return TRUE
