...
Since Running *kruskal's* algorithm for $G$, we got a subset $T$ Minimum Spanning Tree such that set $T$ contains $V-1$ edges that connect all the vertices in $V\in G$ , and this path connect all $v \in V$ by $e\in T$ is the minimum path given by the *Cut property*. Therefore, $G(V,T)$ is a sub-forest, and any tree for this sub-forest has no path to each other, then it satisfies the structure of result.

Now, let $g$ be the heaviest edge as the first greedy decision in $T$. Let $AS\subseteq T$ be an arbitrary solution that does **NOT** contain $g$. Then the solution $AS'\subseteq T$ that contains $g$ and $AS'$ is at least as good as $AS$.

To prove that $AS'$ has valid structure, we could use the *Cut property* of MSTs. Then, after cutting $k-1$ edges from $T$, it still remains a sub-forest of $G$. It is a valid structure and the value of $|AS'| \ge |AS|$.
