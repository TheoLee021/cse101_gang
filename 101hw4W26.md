...
Run Kruskal Algorithm on G. Maintain a list K of all edges rejected by Kruskal. Since Kruskal returns the MST, every rejected edge will create a cycle, so we only need to return the heaviest edge in R. If there are no edges in R, then there cannot be any cycles. Because Kruskal picks the lightest edges first, there is no chance that an edge in MST can be heavier than every edge in R.

- Correctness Proof:

  WTS: Every edge rejected by Kruskal will create a cycle if added.

  Proof: When Kruskal encounters an edge e = (u,v) and find(u) = find(v), it will reject e. This makes sense because there is already a path between u and v, so adding e will create a cycle. Thus, all edges rejected will create a cycle if added.

  WTS: The heaviest edge that is part of a cycle cannot be in the MST returned by Kruskal

  Proof: Let e' be an edge in the the result MST that is the heaviest edge in G . Kruskal will always consider the lightest edge first, so that means e' will be considered last. Therefore, there can only be 2 cases. Case 1, there is already a lighter edge that connects both vertices of e'. Adding e' will then create a cycle, so Kruskal rejects e' (proved). Case 2, there is no lighter edge that connects both vertices of e'. Then adding e' will not create a cycle. Then e' is not part of a cycle, which contradicts our assumption.
